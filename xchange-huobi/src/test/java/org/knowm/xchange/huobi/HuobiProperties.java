...
class HuobiProperties {
	...
	public HuobiProperties() throws IOException {
		...
		try (InputStream input = getClass().getResourceAsStream(FILENAME)) {
			...
			if (apiKey == null || apiKey.isEmpty()) { throw new IllegalArgumentException("API key is missing"); }
			if (secretKey == null || secretKey.isEmpty()) { throw new IllegalArgumentException("Secret key is missing"); }
			valid = true;
