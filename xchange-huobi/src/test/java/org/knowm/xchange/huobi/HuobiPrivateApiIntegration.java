...
public class HuobiPrivateApiIntegration {
	...
	private String placePendingOrder() throws IOException {
		try {
			...
			HuobiAccountService accountService = (HuobiAccountService) exchange.getAccountService();
			HuobiAccount[] accounts = accountService.getAccounts();
			LimitOrder limitOrder = new LimitOrder(OrderType.BID, new BigDecimal("1.9"), new CurrencyPair("NAS", "ETH"), String.valueOf(accounts[0].getId()), null,
	...
	public void placeMarketOrderTest() throws IOException {
		...
		HuobiAccountService accountService = (HuobiAccountService) exchange.getAccountService();
		HuobiAccount[] accounts = accountService.getAccounts();
		MarketOrder marketOrder = new MarketOrder(OrderType.ASK, new BigDecimal("0.0002"), new CurrencyPair("BTC", "USDT"), String.valueOf(accounts[0].getId()), null);
